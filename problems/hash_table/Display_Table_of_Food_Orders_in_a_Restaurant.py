...
restaurant. More specifically orders[i]=[customerNamei,tableNumberi,foodItemi]
...
entries denote how many of each food item each table ordered. The first column is
the table number and the remaining columns correspond to each food item in alphabetical
order. The first row should be a header whose first column is “Table”, followed by
the names of the food items. Note that the customer names are not part of the table.
...
class Solution:
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        ...
        for _, t, f in orders:
            ...
            food.add(f)
        res = []
        table, food = sorted(table, key=lambda x: int(x)), sorted(food)
