...
Explanation:
The output consists of two word squares. The order of output does not matter (just
...
class Solution:
    def wordSquares(self, words: List[str]) -> List[List[str]]:
        ...
        res = []
        self.backtrack(n, 0, d, [], res)
        return res

    def backtrack(self, n, i, d, temp, res):
        if i == n:
            res.append(temp[:])
            return

        prefix = ''.join([t[i] for t in temp])
        for word in d[prefix]:
            temp.append(word)
            self.backtrack(n, i + 1, d, temp, res)
            temp.pop()
