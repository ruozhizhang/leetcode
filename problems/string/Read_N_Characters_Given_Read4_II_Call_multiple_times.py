...
from the file.
sol.read(buf, 1); // After calling your read method, buf should contain "a". We read a total
of 1 character from the file, so return 1.
sol.read(buf, 2); // Now buf should contain "bc". We read a total of 2 characters from the
file, so return 2.
sol.read(buf, 1); // We have reached the end of file, no more characters can be read. So
return 0.
...
class Solution:
    ...
    def read(self, buf, n):
        ...
        """
        # step 1, call read4 until self.s has enough chars to return or no more chars
        # are left in the file
        l = 4
        while len(self.s) < n and l == 4:
            l = read4(buf)
            self.s += ''.join(buf[:l])

        # step 2, return the needed chars, leave the rest in self.s
        m = min(len(self.s), n)
        for i in range(m):
            buf[i] = self.s[i]
        self.s = self.s[m:]
