...
The first cut is done to a rod of length 7 so the cost is 7. The second cut is done to a rod
of length 6 (i.e. the second part of the first cut), the third is done to a rod of length 4
and the last cut is to a rod of length 3. The total cost is 7 + 6 + 4 + 3 = 20.
...
class Solution:
    def minCost(self, n: int, cuts: List[int]) -> int:
        cuts += [0, n]
        cuts.sort()
        return self.helper(cuts, 0, len(cuts) - 1, {})

    def helper(self, cuts, l, r, cache):
        if (l, r) in cache:
            return cache[(l, r)]
        ...
        for k in range(l + 1, r):
            res = min(res, cuts[r] - cuts[l] + self.helper(cuts, l, k, cache) + self.helper(cuts, k, r, cache))
        cache[(l, r)] = res
        return res
